...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_29_510[0]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_29_510[1]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_29_510[2]";
A_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 0 ;
aux_cmp_num += 0;
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[1]); // line circom 28
