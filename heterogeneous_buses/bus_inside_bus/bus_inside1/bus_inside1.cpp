...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_42_691[0]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_42_691[1]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_42_691[2]";
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[4]); // line circom 41
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + ((4 * Fr_toInt(&lvar[4])) + 12)],4);
}
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((4 * Fr_toInt(&lvar[4])) + 0)];
// load src
// end load src
Fr_copyn(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 0)]].signalStart + ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 0)]].templateId].defs[0].offset],4);
}
{
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[5]); // line circom 41
// end load src
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[5]); // line circom 41
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[4]); // line circom 41
