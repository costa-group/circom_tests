...
for (uint i = 0; i < 3; i++) {
std::string new_cmp_name = "A_29_494"+ctx->generate_position_array(aux_dimensions, 1, i);
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[2]); // line circom 28
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + ((2 * Fr_toInt(&lvar[4])) + 6)],2);
}
...
// load src
// end load src
Fr_copyn(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 0)]].signalStart + 0],2);
}
{
PFrElement aux_dest = &lvar[4];
// load src
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[2]); // line circom 28
