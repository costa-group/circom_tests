...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_28_468[0]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_28_468[1]";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "A_28_468[2]";
A_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 0 ;
aux_cmp_num += 0;
...
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[4]); // line circom 27
// end load src
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[4]); // line circom 27
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[1]); // line circom 27
