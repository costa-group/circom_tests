...
Fr_eq(&expaux[0],&lvar[18],&circuitConstants[8]); // line circom 64
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[20];
// load src
...
Fr_lt(&expaux[0],&lvar[20],&circuitConstants[3]); // line circom 65
while(Fr_isTrue(&expaux[0])){
{
...
{
uint cmp_index_ref = 1;
std::map<int,int> size_store {{2,32},{0,2},{1,2}};
{
...
PFrElement aux_dest = &lvar[20];
// load src
Fr_add(&expaux[0],&lvar[20],&circuitConstants[8]); // line circom 65
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[20],&circuitConstants[3]); // line circom 65
...
std::map<int,int> size_store {{2,5},{0,6},{1,7}};
{
...
{
uint cmp_index_ref = 2;
std::map<int,int> size_store {{2,160},{0,12},{1,14}};
{
uint map_accesses_aux[2];
{
IOFieldDef *cur_def = &(ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[cmp_index_ref]].templateId].defs[1]);
...
Fr_lt(&expaux[0],&lvar[20],&lvar[((1 * Fr_toInt(&lvar[18])) + 9)]); // line circom 73
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[20];
// load src
Fr_add(&expaux[0],&lvar[20],&circuitConstants[8]); // line circom 73
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[20],&lvar[((1 * Fr_toInt(&lvar[18])) + 9)]); // line circom 73
