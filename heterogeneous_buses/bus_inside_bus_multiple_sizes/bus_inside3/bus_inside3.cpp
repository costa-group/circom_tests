...
std::map<int,int> size_store {{1,12},{0,8},{2,20}};
{
...
Fr_eq(&expaux[0],&lvar[6],&circuitConstants[3]); // line circom 53
if(Fr_isTrue(&expaux[0])){
{
...
Fr_lt(&expaux[0],&lvar[7],&circuitConstants[1]); // line circom 54
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[7];
// load src
Fr_add(&expaux[0],&lvar[7],&circuitConstants[3]); // line circom 54
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[7],&circuitConstants[1]); // line circom 54
