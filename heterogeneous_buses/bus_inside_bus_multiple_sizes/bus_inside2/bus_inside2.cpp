...
std::map<int,int> size_store {{0,4},{1,6},{2,3}};
{
...
Fr_lt(&expaux[0],&lvar[13],&circuitConstants[0]); // line circom 52
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
std::map<int,int> size_store {{0,3},{1,5},{2,9}};
{
...
PFrElement aux_dest = &lvar[13];
// load src
Fr_add(&expaux[0],&lvar[13],&circuitConstants[6]); // line circom 52
// end load src
...
Fr_eq(&expaux[0],&lvar[12],&circuitConstants[6]); // line circom 55
if(Fr_isTrue(&expaux[0])){
{
...
{
uint cmp_index_ref = 1;
std::map<int,int> size_store {{0,12},{1,30},{2,27}};
{
uint map_accesses_aux[1];
{
IOFieldDef *cur_def = &(ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[cmp_index_ref]].templateId].defs[1]);
...
Fr_lt(&expaux[0],&lvar[13],&lvar[((1 * Fr_toInt(&lvar[12])) + 6)]); // line circom 62
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[13];
// load src
Fr_add(&expaux[0],&lvar[13],&circuitConstants[6]); // line circom 62
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[13],&lvar[((1 * Fr_toInt(&lvar[12])) + 6)]); // line circom 62
