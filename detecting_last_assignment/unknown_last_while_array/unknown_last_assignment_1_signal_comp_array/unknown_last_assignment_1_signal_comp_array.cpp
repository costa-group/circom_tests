...
uint get_size_of_witness() {return 30;}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 18]); // line circom 15
// end load src
...
C_1_create(mySignalStart+23,1+ctx_index+1,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 30
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((3 * Fr_toInt(&lvar[1])) + 1)];
// load src
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((3 * Fr_toInt(&lvar[1])) + 10)];
// load src
cmp_index_ref_load = 1;
cmp_index_ref_load = 1;
// end load src
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 30
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 30
