...
uint get_size_of_witness() {return 72;}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 61]); // line circom 6
// end load src
...
B_0_create(mySignalStart+7,0+ctx_index+1,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 20
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[1])) + 0) + 22)];
// load src
// end load src
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[1])) + 3) + 22)];
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 4],3);
}
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[4]); // line circom 20
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 20
