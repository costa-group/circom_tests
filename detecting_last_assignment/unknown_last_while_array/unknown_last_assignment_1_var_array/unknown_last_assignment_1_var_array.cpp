...
uint get_total_signal_no() {return 105;}

uint get_number_of_components() {return 2;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 105;}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 97]); // line circom 8
// end load src
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[0]); // line circom 21
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[4])) + 3) + 34)];
// load src
// end load src
Fr_copyn(aux_dest,&lvar[1],3);
}
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[4])) + 0) + 34)];
// load src
// end load src
Fr_copyn(aux_dest,&lvar[1],3);
}
...
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[2]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[0]); // line circom 21
