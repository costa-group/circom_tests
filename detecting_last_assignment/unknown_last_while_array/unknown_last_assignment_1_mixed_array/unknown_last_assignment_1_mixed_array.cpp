...
void C_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void C_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 57;}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 43]); // line circom 15
// end load src
...
B_0_create(mySignalStart+4,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "a1";
C_1_create(mySignalStart+50,1+ctx_index+1,ctx,new_cmp_name,myId);
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 1],3);
}
{
...
C_1_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[0]); // line circom 31
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((3 * Fr_toInt(&lvar[4])) + 1)];
// load src
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[4])) + 3) + 16)];
// load src
// end load src
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((6 * Fr_toInt(&lvar[4])) + 0) + 16)];
// load src
cmp_index_ref_load = 1;
cmp_index_ref_load = 1;
// end load src
...
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[4]); // line circom 31
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[0]); // line circom 31
