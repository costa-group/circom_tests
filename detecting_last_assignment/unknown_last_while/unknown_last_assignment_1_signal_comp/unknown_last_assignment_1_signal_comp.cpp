...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 6]); // line circom 15
// end load src
...
C_1_create(mySignalStart+9,1+ctx_index+1,ctx,new_cmp_name,myId);
...
cmp_index_ref_load = 1;
cmp_index_ref_load = 1;
Fr_add(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[3]); // line circom 33
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[4]); // line circom 30
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 30
