...
// load src
// end load src
Fr_copyn(aux_dest,&lvar[83],10);
}
{
PFrElement aux_dest = &lvar[53];
// load src
// end load src
Fr_copyn(aux_dest,&lvar[93],10);
}
{
PFrElement aux_dest = &lvar[63];
// load src
// end load src
Fr_copyn(aux_dest,&lvar[103],10);
}
{
...
Fr_lt(&expaux[0],&lvar[113],&circuitConstants[5]); // line circom 43
...
// copying argument 0
Fr_copy(&lvarcall[0],&lvar[113]);
// end copying argument 0
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[113])) + 0);
...
uint aux_src_index = ((1 * Fr_toInt(&lvar[113])) + 0);
std::map<int,int> size_src_store {{1,4},{2,6},{0,2}};
{
PFrElement aux_dest = &lvar[((10 * Fr_toInt(&lvar[113])) + 3)];
// load src
// end load src
Fr_copyn(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[113])) + 0)]].signalStart + ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[113])) + 0)]].templateId].defs[1].offset],std::min(10, size_src_store[ctx->componentMemory[mySubcomponents[aux_src_index]].templateId]));
}
{
...
Fr_lt(&expaux[0],&lvar[114],&circuitConstants[11]); // line circom 52
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[114];
// load src
Fr_add(&expaux[0],&lvar[114],&circuitConstants[4]); // line circom 52
// end load src
...
PFrElement aux_dest = &lvar[113];
// load src
Fr_add(&expaux[0],&lvar[113],&circuitConstants[4]); // line circom 43
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[113],&circuitConstants[5]); // line circom 43
}
Fr_eq(&expaux[0],&lvar[3],&lvar[43]); // line circom 58
index_multiple_eq = 1;
while(index_multiple_eq < 30 && Fr_isTrue(&expaux[0])) {
Fr_eq(&expaux[0],&lvar[3] + index_multiple_eq,&lvar[43] + index_multiple_eq); // line circom 58
index_multiple_eq++;
}
