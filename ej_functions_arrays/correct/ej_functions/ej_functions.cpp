...
void foo_0(Circom_CalcWit* ctx,FrElement* lvar,uint componentFather,FrElement* destination,int destination_size);
...
// load src
Fr_mul(&expaux[1],&circuitConstants[4],&circuitConstants[3]); // line circom 5
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[5]); // line circom 5
// end load src
...
foo_0(ctx,lvarcall,myId,&lvar[0],2);
// end call bucket
}
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[2]); // line circom 18
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 0; i++){
