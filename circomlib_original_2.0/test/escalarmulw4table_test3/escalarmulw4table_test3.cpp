...
Fr_lt(&expaux[0],&lvar[36],&circuitConstants[18]); // line circom 13
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[36])) + 0) + 0)];
// load src
Fr_mul(&expaux[0],&lvar[(((2 * Fr_toInt(&lvar[36])) + 0) + 2)],&signalValues[mySignalStart + 32]); // line circom 14
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&lvar[(((2 * Fr_toInt(&lvar[36])) + 1) + 2)],&signalValues[mySignalStart + 32]); // line circom 15
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[36];
// load src
Fr_add(&expaux[0],&lvar[36],&circuitConstants[1]); // line circom 13
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[36],&circuitConstants[18]); // line circom 13
