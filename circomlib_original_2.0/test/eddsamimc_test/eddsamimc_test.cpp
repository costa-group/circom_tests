...
Circom_TemplateFunction _functionTable[28] = { 
Num2Bits_0_run,
Num2Bits_1_run,
CompConstant_2_run,
MiMC7_3_run,
...
Circom_TemplateFunction _functionTableParallel[28] = { 
NULL,
NULL,
NULL,
NULL,
...
uint get_total_signal_no() {return 21736;}

uint get_number_of_components() {return 2000;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 21736;}
...
Fr_lt(&expaux[0],&lvar[93],&circuitConstants[96]); // line circom 131
if(Fr_isTrue(&expaux[0])){
{
...
// load src
Fr_mul(&expaux[3],&circuitConstants[189],&signalValues[mySignalStart + 6]); // line circom 48
...
Fr_mul(&expaux[4],&circuitConstants[189],&signalValues[mySignalStart + 6]); // line circom 49
...
// load src
Fr_mul(&expaux[3],&circuitConstants[9],&signalValues[mySignalStart + 5]); // line circom 137
...
Fr_mul(&expaux[2],&circuitConstants[7],&signalValues[mySignalStart + 3]); // line circom 138
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[2]); // line circom 138
Fr_mul(&expaux[4],&circuitConstants[9],&signalValues[mySignalStart + 5]); // line circom 138
...
// load src
Fr_sub(&expaux[2],&circuitConstants[200],&signalValues[mySignalStart + 256]); // line circom 162
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[cmp_index_ref]].templateId].defs[3].offset+map_accesses_aux[0]];
// load src
Fr_sub(&expaux[2],&circuitConstants[201],&signalValues[mySignalStart + 257]); // line circom 163
cmp_index_ref_load = 5;
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[89]); // line circom 166
while(Fr_isTrue(&expaux[0])){
{{
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[9]); // line circom 180
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 1);
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[88]); // line circom 183
if(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 167);
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[89]); // line circom 194
while(Fr_isTrue(&expaux[0])){
{{
...
Fr_idiv(&expaux[1],&expaux[2],&circuitConstants[9]); // line circom 255
...
Fr_mul(&expaux[1],&lvar[8],&circuitConstants[9]); // line circom 263
...
EdDSAMiMCVerifier_27_create(1,0,ctx,"main",0);
EdDSAMiMCVerifier_27_run(0,ctx);
}
