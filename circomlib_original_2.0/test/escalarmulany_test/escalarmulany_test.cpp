...
void Main_12_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Main_12_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 8134;}

uint get_number_of_components() {return 1021;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 8134;}
...
// load src
Fr_sub(&expaux[2],&circuitConstants[16],&signalValues[mySignalStart + 255]); // line circom 162
...
Fr_add(&expaux[0],&signalValues[mySignalStart + 255],&expaux[1]); // line circom 162
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[cmp_index_ref]].templateId].defs[3].offset+map_accesses_aux[0]];
// load src
Fr_sub(&expaux[2],&circuitConstants[17],&signalValues[mySignalStart + 256]); // line circom 163
cmp_index_ref_load = 5;
Fr_mul(&expaux[1],&expaux[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[5]].signalStart + 0]); // line circom 163
Fr_add(&expaux[0],&signalValues[mySignalStart + 256],&expaux[1]); // line circom 163
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "escalarMulAny";
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 21
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 21
