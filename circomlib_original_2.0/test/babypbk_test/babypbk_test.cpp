...
void BabyPbk_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BabyPbk_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 10115;}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "pvkBits";
...
BabyPbk_11_create(1,0,ctx,"main",0);
BabyPbk_11_run(0,ctx);
}
