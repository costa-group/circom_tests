...
#include "circom.hpp"
#include "calcwit.hpp"
void BabyCheck_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BabyCheck_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
Fr_mul(&expaux[2],&circuitConstants[0],&signalValues[mySignalStart + 2]); // line circom 82
Fr_add(&expaux[1],&expaux[2],&signalValues[mySignalStart + 3]); // line circom 82
Fr_mul(&expaux[5],&circuitConstants[1],&signalValues[mySignalStart + 2]); // line circom 82
...
BabyCheck_0_create(1,0,ctx,"main",0);
BabyCheck_0_run(0,ctx);
}
