...
void BinSum_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BinSum_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
Fr_lt(&expaux[0],&lvar[6],&circuitConstants[3]); // line circom 80
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
...
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[4]); // line circom 87
while(Fr_isTrue(&expaux[0])){
{
...
BinSum_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 97 ;
aux_cmp_num += 1;
}
...
BinSum_1_run(mySubcomponents[cmp_index_ref],ctx);
