...
#include "circom.hpp"
#include "calcwit.hpp"
void MontgomeryDouble_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MontgomeryDouble_0_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[1] = { 
MontgomeryDouble_0_run };
...
// load src
Fr_mul(&expaux[3],&circuitConstants[5],&signalValues[mySignalStart + 5]); // line circom 137
Fr_mul(&expaux[4],&circuitConstants[6],&signalValues[mySignalStart + 2]); // line circom 137
...
Fr_mul(&expaux[4],&circuitConstants[5],&signalValues[mySignalStart + 5]); // line circom 138
Fr_mul(&expaux[5],&circuitConstants[6],&signalValues[mySignalStart + 2]); // line circom 138
...
Fr_sub(&expaux[1],&expaux[2],&circuitConstants[2]); // line circom 140
...
MontgomeryDouble_0_create(1,0,ctx,"main",0);
MontgomeryDouble_0_run(0,ctx);
}
