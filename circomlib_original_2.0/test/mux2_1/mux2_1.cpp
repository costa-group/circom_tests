...
#include "circom.hpp"
#include "calcwit.hpp"
void MultiMux2_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MultiMux2_0_run(uint ctx_index,Circom_CalcWit* ctx);
void Mux2_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Mux2_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
MultiMux2_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[4]); // line circom 54
while(Fr_isTrue(&expaux[0])){
{
...
MultiMux2_0_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[3]); // line circom 58
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mux";
Mux2_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 19 ;
aux_cmp_num += 2;
}
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[3]); // line circom 27
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Mux2_1_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[4]); // line circom 30
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 30
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[4]); // line circom 30
