...
Fr_sub(&expaux[2],&signalValues[mySignalStart + 0],&circuitConstants[0]); // line circom 33
Fr_mul(&expaux[1],&signalValues[mySignalStart + 0],&expaux[2]); // line circom 33
{{
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 26
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 26
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 26
