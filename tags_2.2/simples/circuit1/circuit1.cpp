...
#include "circom.hpp"
#include "calcwit.hpp"
void error_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void error_0_run(uint ctx_index,Circom_CalcWit* ctx);
void error_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void error_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "error";
...
uint sub_component_aux;
uint index_multiple_eq;
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&signalValues[mySignalStart + 1]); // line circom 11
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "error_22_234";
error_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "error_27_305";
error_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 2 ;
aux_cmp_num += 1;
}
...
error_0_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
error_1_run(mySubcomponents[cmp_index_ref],ctx);
}
for (uint i = 0; i < 2; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
if (index_subc != 0)release_memory_component(ctx,index_subc);
