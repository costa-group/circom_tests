...
uint get_total_signal_no() {return 87;}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "B_60_836";
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 32],3);
}
{
...
// load src
// end load src
Fr_copyn(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],8);
}
for (uint i = 0; i < 1; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
if (index_subc != 0)release_memory_component(ctx,index_subc);
