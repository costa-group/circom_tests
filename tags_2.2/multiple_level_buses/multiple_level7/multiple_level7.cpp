...
uint get_total_signal_no() {return 84;}
...
Fr_eq(&expaux[0],&circuitConstants[0],&circuitConstants[0]); // line circom 32
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "C_23_295";
C_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
...
Fr_eq(&expaux[0],&circuitConstants[2],&circuitConstants[2]); // line circom 22
...
Fr_eq(&expaux[0],&circuitConstants[3],&circuitConstants[3]); // line circom 25
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "D_59_767";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "B_72_969";
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 54
...
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
B_2_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
