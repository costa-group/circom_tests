...
template A(){
	...
	log(inter.aux.b);
	log(inter.aux.aux.a);
	signal {c} out2[inter.c]; //using the value of inter.c to define the size of out2
	out2.c = inter.c + inter.aux.aux.a;
	for(var i = 0; i < inter.c; i++){
		out2[i] <== 0;
	}
	log(out2.c);
...
template B(){
	...
	a[0].inter[0] <== prueba;
	log(a[0].out.aux[0].aux.a); //It fails since tags should not be used with array accesses but in the component array.
	log(a[0].out.aux.aux[0].a); //it fails since tags should not be used with array accesses but in the component array.
	log(a[0].out[0].aux.aux.a); //it fails since tags should not be used with array accesses but in the component array.
